...
├── 原作者/IP 确权                已有规则、私有快照和重启后状态转换；缺原作者注册表与采集器
├── 权威钱包动作                 已有链上语义规则和幂等 receipt 入口；缺权威钱包注册表与扫描器
├── 现实世界源事件               已有规则和幂等审核入口；缺可信新闻/活动适配器
├── 分发入口打开                 已有状态转换与持久快照；缺交易所/钱包/Launchpad 适配器
├── 风险突然解除                 已有保留否认历史的持久状态机；缺争议与修复证据适配器
└── 正统 CA 胜出                 已有持久竞争决策与 leader-change 事件；缺跨模块聚合和迁移适配器
```

六个新入口共用 `JsonEvidenceStateStore`：目录权限 `0700`、状态和锁文件 `0600`，进程重启后仍能比较前后状态；统一事件库继续负责确定性去重。它们是明确的应用边界，不代表外部数据源已接好。
