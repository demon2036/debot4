...
def prepare_queue_schema(db: sqlite3.Connection) -> None:
    """Create v3 or rebuild an older schema without dropping queued work."""
    ...
    if "priority" in columns and "passive_market_anomaly" in sql:
...
def _migrate_legacy(db: sqlite3.Connection) -> None:
    ...
    try:
        ...
        db.execute(
            ...
            "SELECT job_id,kind,50,content_sha256,payload_json,status,attempts,"
            "max_attempts,available_at,lease_owner,lease_id,lease_expires_at,"
            "error_type,created_at,updated_at,completed_at "
